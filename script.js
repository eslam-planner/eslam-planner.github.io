...
const firebaseConfig = {
    ...
    storageBucket: "eslam-planner.firebasestorage.app",
...
const i18n = {
    ar: {
        ...
        title_finance: "المتتبع المالي", btn_add_trans: "معاملة", fin_inc: "الدخل", fin_exp: "المصروفات", fin_bal: "الرصيد",
        ...
        title_pom: "مؤقت التركيز (Pomodoro)", btn_work: "عمل", btn_break: "استراحة", btn_start: "ابدأ", btn_pause: "إيقاف", btn_reset: "إعادة",
    ...
    en: {
        ...
        title_finance: "Finance Tracker", btn_add_trans: "Transaction", fin_inc: "Income", fin_exp: "Expense", fin_bal: "Balance",
        ...
        title_pom: "Focus Timer (Pomodoro)", btn_work: "Work", btn_break: "Break", btn_start: "Start", btn_pause: "Pause", btn_reset: "Reset",
...
window.clearDailyTasks = (type) => { 
    ...
    else if (type === 'today') { if(confirm(currentLang==='ar'?'حذف جميع مهام اليوم نهائياً (لن تتمكن من استرجاعها)؟':'Delete all tasks for today permanently?')) { tasks = tasks.filter(t => t.date !== currentTodayStr); saveAll(); renderViews(); } } 
};
window.archiveDashboardToday = () => { if(confirm(currentLang==='ar'?'هل تريد إخفاء مهام اليوم من لوحة الإحصائيات فقط؟ (ستبقى محفوظة في جدول اليوم للرجوع إليها لاحقاً)':'Hide today\'s tasks from Dashboard only? (Kept in schedule)')) { localStorage.setItem('fp_dash_cleared', currentTodayStr); renderDashboard(); } };
window.resetStats = () => { if(confirm(currentLang==='ar'?'هل تريد تصفير الإحصائيات والرسم البياني فقط؟ (ستبقى خططك القديمة محفوظة في جدول اليوم)':'Reset dashboard stats only? (Old plans will be kept)')) { localStorage.setItem('fp_stats_reset', getTodayStr()); renderDashboard(); } };
...
window.resetPassword = async () => { const email = document.getElementById('authEmail').value; const errEl = document.getElementById('authError'); if(!email) { errEl.style.color = 'var(--warning)'; errEl.innerText = currentLang==='ar'?'الرجاء كتابة بريدك الإلكتروني بالأعلى':'Please enter your email above.'; return; } try { await auth.sendPasswordResetEmail(email); errEl.style.color = 'var(--success)'; errEl.innerText = currentLang==='ar'?'تم إرسال رابط الاستعادة بنجاح!':'Reset link sent successfully!'; } catch(err) { errEl.style.color = 'var(--danger)'; errEl.innerText = err.message; } };
...
function initDictation() {
    ...
    if (('webkitSpeechRecognition' in window || 'SpeechRecognition' in window)) {
        ...
        dictationRecognition.onstart = function() { 
            isDictating = true; 
            let btn = currentDictationTarget === 'lib' ? dictateLibBtn : dictateNoteBtn;
            let status = document.getElementById(currentDictationTarget === 'lib' ? 'dictateStatusLib' : 'dictateStatusNote');
            if(btn) { btn.classList.add('recording'); btn.innerHTML = '<i class="fa-solid fa-stop"></i>'; }
            if(status) { status.innerText = currentLang === 'ar' ? 'جاري الاستماع... (اضغط إيقاف للتعديل)' : 'Listening...'; status.style.color = 'var(--danger)'; }
        };
        
        dictationRecognition.onresult = function(event) { 
            let transcript = event.results[event.results.length - 1][0].transcript;
            let targetInput = document.getElementById(currentDictationTarget === 'lib' ? 'libContent' : 'noteContent');
            if(targetInput) { targetInput.value = targetInput.value + (targetInput.value ? ' ' : '') + transcript; }
        };
        
        dictationRecognition.onerror = function(event) { console.error("Dictation error:", event.error); stopDictation(); };
        ...
        const startDictation = (target) => {
            if (isDictating) { isDictating = false; dictationRecognition.stop(); } else { 
                currentDictationTarget = target;
                let langSelect = document.getElementById(target === 'lib' ? 'dictateLangLib' : 'dictateLangNote');
                dictationRecognition.lang = langSelect ? langSelect.value : (currentLang === 'ar' ? 'ar-EG' : 'en-US'); 
        ...
        let msg = currentLang === 'ar' ? 'متصفح الهاتف لا يدعم تحويل الصوت لنص.' : 'Speech-to-Text not supported.';
        if(dictateLibBtn) dictateLibBtn.onclick = () => alert(msg);
        if(dictateNoteBtn) dictateNoteBtn.onclick = () => alert(msg);
    }
}
function stopDictation() { 
    isDictating = false; if(dictationRecognition) dictationRecognition.stop(); 
    ['lib', 'note'].forEach(target => {
        let btn = document.getElementById(target === 'lib' ? 'dictateLibBtn' : 'dictateNoteBtn');
        let status = document.getElementById(target === 'lib' ? 'dictateStatusLib' : 'dictateStatusNote');
        if(btn) { btn.classList.remove('recording'); btn.innerHTML = '<i class="fa-solid fa-microphone"></i>'; } 
        if(status) { status.innerText = currentLang === 'ar' ? i18n.ar.lib_dictate_hint : i18n.en.lib_dictate_hint; status.style.color = 'var(--text-main)'; }
    });
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('taskDate').value = currentDailyDate; document.getElementById('finDate').value = currentTodayStr; document.getElementById('noteDate').value = currentTodayStr;
    const taskHour = document.getElementById('taskHour'); taskHour.innerHTML = ''; for(let i = 8; i <= 21; i++) { let opt = document.createElement('option'); opt.value = i; if(i === 12) opt.textContent = '12 PM'; else if(i > 12) opt.textContent = `${i - 12} PM`; else opt.textContent = `${i} AM`; taskHour.appendChild(opt); }
...
function renderDaily() { 
    ...
    for(let hour = 8; hour <= 21; hour++) { 
        const hourTasks = todayTasks.filter(t => t.hour == hour); let timeLabel; if(hour === 12) timeLabel = '12 PM'; else if(hour > 12) timeLabel = `${hour - 12} PM`; else timeLabel = `${hour} AM`;
...
function renderNotes() { const container = document.getElementById('notesContainer'); container.innerHTML = notes.length === 0 ? `<p style="text-align:center; color:var(--text-muted); grid-column: 1/-1;">${currentLang==='ar'?'لا توجد ملاحظات.':'No notes found.'}</p>` : ''; notes.forEach(note => { container.innerHTML += `<div class="note-card"><button class="delete-note no-print" onclick="deleteNote(${note.id})"><i class="fa-solid fa-trash"></i></button><span class="note-date"><i class="fa-solid fa-calendar"></i> ${note.date}</span><h3 style="margin-bottom: 0.5rem;">${note.title}</h3><p style="color:var(--text-muted); font-size:0.95rem; white-space: pre-wrap;">${note.content}</p></div>`; }); }
document.getElementById('saveNoteBtn').onclick = () => { const t = document.getElementById('noteTitle').value, c = document.getElementById('noteContent').value, d = document.getElementById('noteDate').value; if(!t && !c) return; notes.push({ id: Date.now(), title: t || (currentLang==='ar'?'ملاحظة جديدة':'New Note'), content: c, date: d }); saveAll(); document.getElementById('noteModal').classList.remove('show'); document.getElementById('noteTitle').value=''; document.getElementById('noteContent').value=''; stopDictation(); renderNotes(); };
window.deleteNote = id => { notes = notes.filter(n => n.id !== id); saveAll(); renderNotes(); }

function renderMonthly() { const container = document.getElementById('monthlyContainer'); container.innerHTML = ''; const mNames = currentLang === 'ar' ? monthNamesAr : monthNamesEn; document.getElementById('monthlyTitle').innerText = `${mNames[currentMonthView]} ${currentYearView}`; const daysInMonth = new Date(currentYearView, currentMonthView + 1, 0).getDate(); for(let i = 1; i <= daysInMonth; i++) { let storageKey = `PlannerMonthData_${currentYearView}_${currentMonthView}_${i}`; let savedText = localStorage.getItem(storageKey) || ""; const dayDiv = document.createElement('div'); dayDiv.className = 'month-day-card'; dayDiv.innerHTML = `<div class="month-day-header"><span>${currentLang==='ar'?'اليوم:':'Day:'} ${i} ${mNames[currentMonthView]}</span></div><textarea class="multi-line-input no-print" rows="3" placeholder="${currentLang==='ar'?'اكتب خطتك (الروابط ستعمل تلقائياً)':'Type your plan (links work automatically)'}" data-key="${storageKey}">${savedText}</textarea><div class="render-area">${linkify(savedText)}</div>`; container.appendChild(dayDiv); } document.querySelectorAll('.multi-line-input').forEach(ta => { ta.addEventListener('input', e => { e.target.nextElementSibling.innerHTML = linkify(e.target.value); }); ta.addEventListener('change', e => { localStorage.setItem(e.target.getAttribute('data-key'), e.target.value); saveAll(); }); }); }

let pomTimer, pomTimeLeft = 25 * 60, isPomRunning = false, pomMode = 'work', workDuration = 25;
function initPomodoro() { const d = document.getElementById('timerDisplay'); const wd = document.getElementById('pomWorkDuration'); const updateTimeDisplay = () => { d.innerText = `${Math.floor(pomTimeLeft/60).toString().padStart(2,'0')}:${(pomTimeLeft%60).toString().padStart(2,'0')}`; }; document.getElementById('pomMinus').onclick = () => { if(workDuration > 15) { workDuration -= 5; if(pomMode==='work'){ pomTimeLeft = workDuration*60; updateTimeDisplay();} wd.innerText = workDuration; } }; document.getElementById('pomPlus').onclick = () => { if(workDuration < 40) { workDuration += 5; if(pomMode==='work'){ pomTimeLeft = workDuration*60; updateTimeDisplay();} wd.innerText = workDuration; } }; const setMode = (m, mins) => { clearInterval(pomTimer); isPomRunning=false; pomMode=m; pomTimeLeft=mins*60; updateTimeDisplay(); document.getElementById('pomWork').classList.toggle('active', m==='work'); document.getElementById('pomBreak').classList.toggle('active', m==='break'); }; document.getElementById('pomWork').onclick = () => setMode('work', workDuration); document.getElementById('pomBreak').onclick = () => setMode('break', 5); document.getElementById('pomStart').onclick = () => { if(isPomRunning) return; isPomRunning = true; pomTimer = setInterval(() => { pomTimeLeft--; updateTimeDisplay(); if(pomTimeLeft<=0) { clearInterval(pomTimer); isPomRunning=false; alert(pomMode==='work'?'Break Time!':'Back to Work!'); setMode(pomMode==='work'?'break':'work', pomMode==='work'?5:workDuration); } }, 1000); }; document.getElementById('pomPause').onclick = () => { clearInterval(pomTimer); isPomRunning=false; }; document.getElementById('pomReset').onclick = () => setMode(pomMode, pomMode==='work'?workDuration:5); updateTimeDisplay(); }
...
window.addKanbanItem = () => { const inp = document.getElementById('newKbItem'); const t = inp.value.trim(); if(t) { kanbanTasks.todo.push({id:Date.now(), text:t}); inp.value = ''; saveAll(); renderKanban(); } }
...
document.getElementById('saveFinBtn').onclick = () => { let desc = document.getElementById('finDesc').value; let amt = document.getElementById('finAmount').value; if(!desc || !amt) return; finances.push({ id: Date.now(), desc: desc, amount: amt, type: document.getElementById('finType').value, date: document.getElementById('finDate').value }); saveAll(); document.getElementById('financeModal').classList.remove('show'); document.getElementById('finDesc').value=''; document.getElementById('finAmount').value=''; renderFinance(); renderDashboard(); };
window.delFin = id => { finances = finances.filter(f => f.id !== id); saveAll(); renderFinance(); renderDashboard(); }
function renderLibrary() { const container = document.getElementById('libraryContainer'); container.innerHTML = library.map(l => `<div class="lib-card"><button class="icon-btn no-print" style="position:absolute; top:10px; left:10px; color:var(--danger);" onclick="delLib(${l.id})"><i class="fa-solid fa-trash"></i></button><span class="lib-cat"><i class="fa-solid fa-tag"></i> ${l.category}</span><h3 style="margin-bottom:10px;">${l.title}</h3><div class="render-area" style="font-size:0.85rem; padding:5px;">${linkify(l.content)}</div></div>`).join('') || `<p style="text-align:center; color:var(--text-muted); grid-column: 1/-1;">${currentLang==='ar'?'أضف مرجعك الأول.':'Add your first reference.'}</p>`; }
document.getElementById('saveLibBtn').onclick = () => { let t = document.getElementById('libTitle').value, c = document.getElementById('libCategory').value, text = document.getElementById('libContent').value; if(!t) return; library.push({ id: Date.now(), title: t, category: c || 'عام', content: text }); saveAll(); document.getElementById('libraryModal').classList.remove('show'); document.getElementById('libTitle').value=''; document.getElementById('libCategory').value=''; document.getElementById('libContent').value=''; stopDictation(); renderLibrary(); };
window.delLib = id => { library = library.filter(l => l.id !== id); saveAll(); renderLibrary(); }
function renderHabits() { let dim = new Date(currentYearView, currentMonthView + 1, 0).getDate(); let html = `<table class="habit-table"><thead><tr><th class="habit-name">${currentLang==='ar'?'العادة':'Habit'}</th>`; for(let i=1; i<=dim; i++) html += `<th>${i}</th>`; html += `</tr></thead><tbody>`; habits.forEach(h => { html += `<tr><td class="habit-name"><button class="icon-btn no-print" style="color:red; font-size:0.8rem;" onclick="delHabit(${h.id})"><i class="fa-solid fa-xmark"></i></button> ${h.name}</td>`; for(let i=1; i<=dim; i++) { let k = `${currentYearView}-${currentMonthView}-${i}`; html += `<td><div class="habit-check ${h.days[k]?'done':''}" onclick="toggleHabit(${h.id}, '${k}')"><i class="fa-solid fa-check"></i></div></td>`; } html += `</tr>`; }); document.getElementById('habitsContainer').innerHTML = html + `</tbody></table>`; }
