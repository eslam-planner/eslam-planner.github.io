...
window.resetPassword = async () => { const email = document.getElementById('authEmail').value; const errEl = document.getElementById('authError'); if(!email) { errEl.style.color = 'var(--warning)'; errEl.innerText = 'يرجى كتابة البريد الإلكتروني.'; return; } try { await auth.sendPasswordResetEmail(email); errEl.style.color = 'var(--success)'; errEl.innerText = 'تم إرسال رابط الاستعادة!'; } catch(err) { errEl.style.color = 'var(--danger)'; errEl.innerText = err.message; } };
...
function initDictation() {
    ...
    if (('webkitSpeechRecognition' in window || 'SpeechRecognition' in window)) {
        ...
        dictationRecognition.continuous = true; dictationRecognition.interimResults = false;
        dictationRecognition.onstart = function() { 
            ...
            if(status) { status.innerText = currentLang === 'ar' ? 'جاري الاستماع...' : 'Listening...'; status.style.color = 'var(--danger)'; }
        };
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // تعديل الساعات في نافذة إضافة مهمة (من 6 صباحاً لـ 11 مساءً)
    const taskHour = document.getElementById('taskHour'); taskHour.innerHTML = ''; 
    for(let i = 6; i <= 23; i++) { 
        let opt = document.createElement('option'); opt.value = i; 
        if(i === 12) opt.textContent = '12 PM'; else if(i > 12) opt.textContent = `${i - 12} PM`; else opt.textContent = `${i} AM`; 
...
// تعديل الساعات في جدول اليوم (من 6 صباحاً لـ 11 مساءً)
...
// تعديل الساعات في نافذة تعديل مهمة (من 6 صباحاً لـ 11 مساءً)
window.editTask = (id) => {
...
document.getElementById('updateTaskBtn').onclick = () => {
    ...
    if(task) {
        ...
        if(dateStr !== oldDate) {
            let [y, m, d] = dateStr.split('-'); let storageKey = `PlannerMonthData_${parseInt(y)}_${parseInt(m)-1}_${parseInt(d)}`;
            let currentText = localStorage.getItem(storageKey) || ""; let timeTxt = hour == 12 ? '12 PM' : (hour > 12 ? (hour-12)+' PM' : hour+' AM');
...
function renderFinance() { const container = document.getElementById('financeContainer'); let inc = 0, exp = 0; let html = finances.sort((a,b) => new Date(b.date) - new Date(a.date)).map(f => { if(f.type === 'income') inc += Number(f.amount); else exp += Number(f.amount); let icon = f.type === 'income' ? '<i class="fa-solid fa-arrow-trend-up"></i>' : '<i class="fa-solid fa-arrow-trend-down"></i>'; return `<div class="fin-item"><div><small>${f.date}</small><br><b>${f.desc}</b></div><div style="display:flex; align-items:center; gap:15px;"><span class="fin-amt ${f.type === 'income' ? 'inc' : 'exp'}">${icon} ${f.amount}</span><button class="icon-btn no-print" onclick="delFin(${f.id})"><i class="fa-solid fa-trash"></i></button></div></div>`; }).join(''); document.getElementById('totalIncome').innerText = inc; document.getElementById('totalExpense').innerText = exp; document.getElementById('netBalance').innerText = inc - exp; container.innerHTML = html || `<p style="text-align:center;">لا توجد معاملات.</p>`; }
...
function renderHabits() { let dim = new Date(currentYearView, currentMonthView + 1, 0).getDate(); let html = `<table class="habit-table"><thead><tr><th>العادة</th>`; for(let i=1; i<=dim; i++) html += `<th>${i}</th>`; html += `</tr></thead><tbody>`; habits.forEach(h => { html += `<tr><td class="habit-name"><button class="icon-btn no-print" style="color:red;" onclick="delHabit(${h.id})">x</button> ${h.name}</td>`; for(let i=1; i<=dim; i++) { let k = `${currentYearView}-${currentMonthView}-${i}`; html += `<td><div class="habit-check ${h.days[k]?'done':''}" onclick="toggleHabit(${h.id}, '${k}')">✓</div></td>`; } html += `</tr>`; }); document.getElementById('habitsContainer').innerHTML = html + `</tbody></table>`; }
...
function initProfile() { document.getElementById('profileName').value = profile.name; document.getElementById('saveProfileBtn').onclick = () => { profile.name = document.getElementById('profileName').value; saveAll(); alert("تم الحفظ!"); }; }
